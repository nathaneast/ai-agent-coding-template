...
setup() {
  ...
  cd "$TEST_TMPDIR"
  mkdir -p .claude/hooks .omc/learnings .omc/sessions
  cp "$REPO_ROOT_SRC/.claude/hooks/session-end.sh" .claude/hooks/
...
@test "session-end appends to index.json" {
  cd "$TEST_TMPDIR"
  echo '{"session_id":"test-123","summary":"test session"}' | bash .claude/hooks/session-end.sh
...
@test "session-end creates archive dump" {
  cd "$TEST_TMPDIR"
  echo '{"session_id":"test-456","summary":"test"}' | bash .claude/hooks/session-end.sh
...
@test "session-end caps index at 50" {
  ...
  for i in $(seq 1 55); do
    echo "{\"session_id\":\"s-$i\",\"summary\":\"s$i\"}" | bash .claude/hooks/session-end.sh
  done
  COUNT=$(jq 'length' .omc/sessions/index.json)
  [ "$COUNT" = "50" ]
}
...
@test "session-end increments learnings_recalled" {
  cd "$TEST_TMPDIR"
  echo '{"session_id":"s","summary":"x"}' | bash .claude/hooks/session-end.sh
